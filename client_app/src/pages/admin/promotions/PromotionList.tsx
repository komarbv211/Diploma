...
const PromotionList: React.FC = () => {
  ...
  const filteredPromotions = useMemo(
    () => filterPromotions(promotions, searchText),
  ...
  const renderActions = (promotion: IPromotion) => {
    const items = [
      {
        key: "edit",
        label: <Link to={`edit/${promotion.id}`}>Редагувати</Link>,
      },
      {
        key: "delete",
        danger: true,
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "startDate",
      render: (date: string) => (
        <Tag color="green">{new Date(date).toLocaleDateString()}</Tag>
      ),
    },
    {
      ...
      key: "endDate",
      render: (date: string) => (
        <Tag color="red">{new Date(date).toLocaleDateString()}</Tag>
      ),
    },
    {
      title: "Зображення",
      ...
      render: (img?: string) =>
        img ? (
          <Image
            width={60}
            src={`${APP_ENV.IMAGES_1200_URL}${img}`}
    ...
    },
    {
      title: "Дії",
      key: "actions",
      render: (_: unknown, record: IPromotion) => renderActions(record),
    },
  ];
            ...
            renderItem={(product: IProduct) => (
              <List.Item key={product.id}>
                ...
                {product.images && product.images.length > 0 && (
                  <Image
                    width={80}
                    src={`${APP_ENV.IMAGES_1200_URL}${product.images[0].name}`}
                  />
                )}
                <Button
                  danger
